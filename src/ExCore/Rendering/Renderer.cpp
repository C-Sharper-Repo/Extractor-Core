...
namespace ExCore {
    ...
    static int32_t textureToShaderType(const Texture* texture) {
        ...
        {
            default: return 0;
            case TextureFormat::Indexed: return 1;
        }
    }

    Renderer::Renderer() : _initialized(false), _window(), _shaders{} { }
    ...
    bool Renderer::initialize(const char* title, uint32_t width, uint32_t height) {
        if (_initialized) {
            EX_CORE_WARN("Warning: Renderer already initialized!");
            return true;
        }
        const int32_t glfwErr = glfwInit();
        if (glfwErr == GLFW_FALSE) {
            EX_CORE_ERROR("Error: Failed to initialize GLFW!");
            terminate();
            return false;
        }

        if (!_window.initialize(title, width, height)) {
            EX_CORE_ERROR("Error: Failed to initialize window!");
            release();
            return false;
        }
        ...
        EX_CORE_TRACE("OpenGL Version: {0}", reinterpret_cast<const char*>(glGetString(GL_VERSION)));
    ...
    bool Renderer::update() { return _window.update(); }
        ...
        if (!_window.isMinimized()) {
            if (cam) {
                ...
                if (_activeRenderers.size() > 0) {
                    ...
                    Matrix4f proj = _window.getWorldProjection();
