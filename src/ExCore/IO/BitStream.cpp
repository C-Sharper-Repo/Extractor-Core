...
size_t BitStream::read(void* buffer, size_t elementSize, size_t count) const {
...
size_t BitStream::write(const void* buffer, const size_t elementSize, const size_t count) const {
...
void BitStream::checkBitBuffer() const {
    ...
    if (_stream) {
        if (_flags & FLAG_NEED_READ) {
            ...
            _stream->read(_bitBuffer, BIT_BUFFER_SIZE);
