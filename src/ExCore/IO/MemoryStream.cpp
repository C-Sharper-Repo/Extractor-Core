...
size_t MemoryStream::read(void* buffer, size_t elementSize, size_t count) const {
    if (!canRead()) { return 0; }
...
size_t MemoryStream::write(const void* buffer, const size_t elementSize, const size_t count) const {
    if (!canWrite()) { return 0; }

    size_t size = elementSize * count;
    if (!tryReserve(_position + size)) { return _position; }
